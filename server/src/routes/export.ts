...
router.post('/batch', async (req, res) => {
  try {
    ...
    res.set({
      ...
      'Content-Disposition': `attachment; filename="documents-export-${date}.zip"`,
    });
...
router.get('/csv', (req, res) => {
  try {
    ...
    res.set({
      ...
      'Content-Disposition': `attachment; filename="documents-index-${new Date().toISOString().split('T')[0]}.csv"`,
    });
    res.send(csv);
  } catch (err: any) {
    res.status(500).json({ error: err.message });
